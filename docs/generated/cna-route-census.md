...
STATUS_DELIBERATE_NON_BINDING=3281
STATUS_UNREVIEWED=0
```
...
| XNA_BACKING | BOUND | 726 | imported: the selected 257-type XNA projection reaches this route |
...
| XNA_BACKING | DELIBERATE_NON_BINDING | 439 | In a family that backs the selected XNA profile, but outside the exact member set the 257-type projection defines; binding it would add native surface with no XNA consumer. |
...
| XNA_BACKING | ACTIONABLE_LOCAL | 2 | XNA's GraphicsDevice constructor and Dispose are public; CNA now has an owned device rather than only the Game's borrowed one. |
